...
public class PSS {
    ...
    private int taskCounter = 1; // Counter for unique IDs
        ...
        for (Task task : tasks) {
            if (task.getId().equals(id)) {
                ...
                if (task instanceof RecurringTask) {
                    // Additional input for recurring task
                    System.out.println("Enter new start date (YYYY-MM-DD):");
                    String newStartDate = scanner.nextLine();
                    System.out.println("Enter new end date (YYYY-MM-DD):");
                    String newEndDate = scanner.nextLine();
                    tasks.remove(task);
                    addTask(new RecurringTask(task.getId(), newName, newStartTime, newDuration, newStartDate, newEndDate));
                } else {
                    tasks.remove(task);
    ...
    public void readTasks() {
        if (tasks.isEmpty()) {
            ...
            for (Task task : tasks) {
                ...
                if (task instanceof TransientTask) {
                    taskDetails += ", Date: " + ((TransientTask) task).date;
                } else if (task instanceof RecurringTask) {
                    taskDetails += ", Start Date: " + ((RecurringTask) task).startDate + ", End Date: " + ((RecurringTask) task).endDate;
                }
        ...
        while (true) {
            ...
            switch (command.toLowerCase()) {
                case "add":
                    ...
                    String id = pss.generateUniqueId();
                    ...
                    if (type.equalsIgnoreCase("recurring")) {
                        ...
                        pss.addTask(new RecurringTask(id, name, startTime, duration, startDate, endDate));
                    } else if (type.equalsIgnoreCase("transient")) {
                        System.out.println("Enter date (YYYY-MM-DD):");
                        String date = scanner.nextLine();
                        pss.addTask(new TransientTask(id, name, startTime, duration, date));
                        ...
                        if (recurringTask != null) {
                            pss.addTask(new AntiTask(id, name, startTime, duration, recurringTask));
                        } else {
                            System.out.println("Recurring task not found.");
                        }
                    } else {
